...
{
  // printf("About to load image file: %s\n", filename);
  ...
  struct ImageHeader * head = malloc(sizeof(struct ImageHeader));

  int x ;  
  x = fread(head, 16,1, fptr) ;

  if(x != 1)
    {
      return NULL ;
    }
  if(head->magic_bits != ECE264_IMAGE_MAGIC_BITS)
    {
      return NULL ;
    } 
  ...
  image1->comment = malloc(sizeof(char) * head->comment_len) ;

  if(image1->comment == NULL)
    {
      return NULL ;
    }
  int y = fread(image1->comment,sizeof(char),head->comment_len,fptr) ;
  ...
  image1->data = malloc(sizeof(uint8_t)* (head->width*head->height)) ;

  if(image1->data == NULL)
    {
      return NULL ;
    }
  int z = fread(image1->data,sizeof(uint8_t),(head->width*head->height),fptr) ;
  ...
  uint8_t  check_data ;
  int a = fread(&check_data,sizeof(uint8_t),1,fptr) ;
  ...
  image1->width = head->width ;

  image1->height = head->height ;
